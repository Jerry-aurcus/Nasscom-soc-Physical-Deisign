...
![WhatsApp Image 2025-07-26 at 17 14 15 (3)](https://github.com/user-attachments/assets/3098ec19-a3b3-4cad-ac1c-dac29c75ae39)


![WhatsApp Image 2025-07-26 at 17 14 16](https://github.com/user-attachments/assets/d72e1be5-2d45-4187-b95d-c2dad551c087)


After this, we are now good to run placement, run_placement.
We will see the iterations, the overflow should decrease with increasing iterations.


<img width="824" height="411" alt="Screenshot 2025-07-26 at 5 55 54 PM" src="https://github.com/user-attachments/assets/b07e9e1f-24ce-4d61-8d8a-73a262c921d3" />

Now lets visualize our placement in magic tool


<img width="825" height="412" alt="Screenshot 2025-07-26 at 5 56 06 PM" src="https://github.com/user-attachments/assets/f163f9be-0ac9-4c67-a6c2-87342efd3854" />


We will find the vsd_inv cell here. We will see the overlapping, means Abettment which means the power and ground rails are shared between the cells.


![WhatsApp Image 2025-07-26 at 17 14 14](https://github.com/user-attachments/assets/671e10db-96a9-4be4-aa26-c75c60696847)


![WhatsApp Image 2025-07-26 at 17 14 16 (2)](https://github.com/user-attachments/assets/4d6f8afa-b4b4-4bb1-bc02-95c4bc42562b)


![WhatsApp Image 2025-07-26 at 17 14 17](https://github.com/user-attachments/assets/996c8069-85f8-44f9-bd7d-94bc080e185f)
...
We begin with **ideal clock timing analysis** to understand the fundamentals. Later, we will introduce **real clocks** for full timing analysis.

Consider a **single clock domain**, where both the **launch flip-flop** and the **capture flip-flop** are triggered by the **same clock**.
...
**Setup analysis** ensures that **data launched** from one flip-flop **arrives at the next flip-flop** before the **clock edge** intended to capture it.

This check is made between two **edges of the same clock** — typically from one rising (or falling) edge to the next.

### Timing Window:

We analyze the timing between `0` and `T` (i.e., **0 ns to 1 ns**).

* At `0 ns`, the **launch flip-flop** receives the clock edge.
* At `1 ns`, the **capture flip-flop** receives the next clock edge.

Let the **combinational delay** be **θ (theta)**.
**Setup timing analysis** requires that:

```plaintext
θ < T
```

This ensures the system functions correctly within the given clock period.


<img width="824" height="411" alt="Screenshot 2025-07-26 at 5 55 54 PM" src="https://github.com/user-attachments/assets/eb95860e-3029-4286-9372-dd5a8fe3dfaa" />


Now we will introduce more practical scenarios in it,
The first thing to be considered is the Flop itself. If we open a particular flop, there is a lot of combinational circuits in it. There are several Mosfets, Capacitances and gates inside it.

<img width="1046" height="401" alt="Screenshot 2025-07-26 at 6 01 07 PM" src="https://github.com/user-attachments/assets/fe166157-d103-40c3-b20a-d81dc8bd29b2" />

### Setup Time in Presence of Multiplexers

Inside the **capture clock path**, there are **two multiplexers**:

* When the **clock = 0**, only **Mux1** contributes to the delay.
* When the **clock = 1**, the delay of **both Mux1 and Mux2** is considered.

There is a **finite time** required for the **D input** to settle before the clock edge triggers. This is known as the **setup time (S)**.

Hence, a time **‘S’** must be maintained **before the clock edge** for valid data to be captured at **Q**.

We can express the delay of **Mux1** as the **setup time (S)**.

Therefore, the timing condition:
...
In practical scenarios, the clock is generated by a **PLL (Phase-Locked Loop)**.
Ideally, the clock edges occur at **t = 0, 2T, 3T, ...**

However, due to the **resistance and capacitance** of the wires used in clock distribution, the actual clock edge may **not occur exactly at the expected time**.

This deviation in the clock edge timing is called **clock jitter**.


<img width="1021" height="235" alt="Screenshot 2025-07-26 at 6 01 46 PM" src="https://github.com/user-attachments/assets/c9e5c89c-ea64-4a19-8f51-25e0ba181585" />


So initially the period was from 0 to T, now it is shrinked to less than 0 and T. This temperory variation even though very less can cause failures in circuit. And also we have kept θ<(T-S), now this will also change. Jitter has to be modelled in the above equation only.
Now let's model Jitter using a second parameter i.e. "Uncertainity". we will club both time for clock edge and uncertainity.

<img width="960" height="277" alt="Screenshot 2025-07-26 at 6 02 39 PM" src="https://github.com/user-attachments/assets/236387db-922d-45ac-b4cb-4fb250c0f6fa" />



So, earlier the combinational delay was supposed to be less than (T-S) will now become θ<(T-S-US), where SU = Setup uncertainity time.

<img width="1024" height="303" alt="Screenshot 2025-07-26 at 6 03 10 PM" src="https://github.com/user-attachments/assets/2e81592f-52b1-4eb4-aea4-635e2b68b8f5" />

Let us take an example; T=1ns, S=10ps=0.01ns, SU=90ps=0.09ns.
The required combinational delay(θ) = 1ns-0.01ns-0.09ns= 0.9ns.
With this timing analysis let's try to identify the timing path from the existing scenario.


<img width="1040" height="478" alt="Screenshot 2025-07-26 at 6 03 52 PM" src="https://github.com/user-attachments/assets/3888a97c-1ae9-4358-a294-431418fb5a60" />

<img width="1058" height="437" alt="Screenshot 2025-07-26 at 6 04 03 PM" src="https://github.com/user-attachments/assets/91ddbc69-457a-4ead-8373-e9033addd2ef" />


### Post-Synthesis Timing Analysis with OpenSTA

From the previous calculation, the expected **combinational delay** is:

```plaintext
θ < 0.9 ns
```

Once the obtained delay satisfies this condition, we confirm that the **placement** is optimized correctly for the **single clock** scenario.

---
## Next Step: Timing Analysis with Multiple Clocks
---

The next challenge is to perform **similar timing analysis** using **multiple clocks**.

### Lab Steps to Configure OpenSTA for Post-Synthesis Timing Analysis

To reduce **slack** and fix **timing violations** in OpenLane, we use a separate tool: **OpenSTA**.

#### Steps:

1. Start the OpenLane Docker session:

```bash
./flow.tcl -interactive
```

2. Inside the session, run the following commands:
...
These steps prepare the design and enable OpenSTA setup for **post-synthesis timing analysis**.
...
![WhatsApp Image 2025-07-26 at 18 09 27 (3)](https://github.com/user-attachments/assets/d108659d-866b-4226-a3fc-1fb50d6ff1b3)

The changes in above file is same as in base.sdc which is in folder openlane/scripts.
The pre_sta.conf file is the file where we'll be doing the Pre-layout Timing Analysis.

We will do sta pre_sta.conf as shown below.


![WhatsApp Image 2025-07-26 at 18 09 27](https://github.com/user-attachments/assets/26a59c53-1a99-4280-ae75-70176cec287c)



![WhatsApp Image 2025-07-26 at 18 09 27 (6)](https://github.com/user-attachments/assets/5e97b221-c6f9-4f75-95ee-d5a80facdaed)

---
## Lab Steps to Optimize Synthesis and Reduce Setup Violations
---


The **delay** of a cell depends on two main factors:

* **Input slew** – higher slew increases delay.
* **Output load** – higher capacitance increases delay.

To reduce delay and setup violations, we **optimize the fanout** by setting:
...
1. Start the OpenLane interactive session:
...
This sets the **fanout to 4** and runs synthesis with optimized constraints to reduce **setup violations**.
